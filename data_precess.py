...
def getPairs(senc_file, label_file):
    ...
    for label in labels:  # 遍历三元组
        lab = entities_dict[label[0]]  # 实体类别缩写
        start = label[1]  # 起始位置
        end = label[2]  # 终止位置
        if end - start == 1:  # 由于标注错误，可能会导致有些位置已经被标记了
            check_single(start, lab)  # sentence[start][1]="S-"+lab
            continue
